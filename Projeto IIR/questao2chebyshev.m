...
figure,freqz(cnum_low,cden_low);
 
s_f_low = filter(cnum_low,cden_low,s);
S_F_low = fft(s_f_low);
S_F_low = abs(2*S_F_low/L);
S_F_low=fftshift(S_F_low);  

figure,freqz(cnum_high,cden_high);
 
s_f_high = filter(cnum_high,cden_high,s);
...
s_f_mid = filter(cnum_mid,cden_mid,s);
S_F_mid = fft(s_f_mid);
